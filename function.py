...
# 関数の構造を管理するクラス
class Function:
    def __init__(self, name, ftype, params, entry, idtable, insts):
        self.program = None
        self.name = name
        self.ftype = ftype
        self.params = params
...
class IdTable:
    ...
    # 識別子表に新しい識別子を追加する。
    def add_id(self, name, kind, attrs={}):
    ...
    # 識別子の持つ属性を取得する。
    def get_id(self, name, attr=None):
    ...
    def id_enumerator(self, **kwargs):
        for name in self.table:
            entry = self.get_id(name)
            match = True
